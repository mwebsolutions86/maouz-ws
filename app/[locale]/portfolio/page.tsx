...
export default function PortfolioPage() {
  ...
  const projects = [
    {
      id: "01", title: "NEXUS AI FIT", category: "Mobile App • HealthTech",
      description: t('project_1_desc'),
      tech: ["React Native", "TensorFlow", "Supabase"], image: "/NexusAIfit.webp", link: "#"
    },
    {
      id: "02", title: "AERO STREAM", category: "Drone Control • IoT",
      description: t('project_2_desc'),
      tech: ["Electron", "WebSockets", "Three.js"], image: "/HardSoftShync.webp", link: "#"
    },
    {
      id: "03", title: "QUANTUM FINANCE", category: "SaaS • Fintech",
      description: t('project_3_desc'),
      tech: ["Next.js", "D3.js", "Go"], image: "/quantume trading.webp", link: "#"
    },
    {
      id: "04", title: "NEON MARKET", category: "E-commerce • Web3",
      description: t('project_4_desc'),
      tech: ["Solidity", "Wagmi", "Tailwind"], image: "/NeonMarket.webp", link: "#"
    }
  ];
      ...
      <div className="max-w-7xl mx-auto relative z-10 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
           {projects.map((project, index) => (
             <motion.div key={index} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: index * 0.1 }} className={`${index % 2 !== 0 ? 'md:mt-24' : ''}`}>
                <Link href={project.link} className="group block relative">
                    <div className="relative h-[300px] sm:h-[400px] md:h-[500px] w-full overflow-hidden rounded-2xl border border-white/10 bg-white/5">
                        ...
                        <Image src={project.image} alt={project.title} fill className="object-cover transition-transform duration-700 group-hover:scale-110 opacity-80 group-hover:opacity-100" unoptimized />
                    </div>
                    <div className="mt-6 md:mt-8 px-2">
                        <div className="flex justify-between items-start mb-2">
                            <div className="text-cyan-500 font-mono text-[10px] md:text-xs font-bold mb-2 tracking-widest">{project.id} {t('category_prefix')} {project.category}</div>
                        ...
                        <p className="text-gray-400 text-sm md:text-base font-light leading-relaxed mb-4 max-w-md">{project.description}</p>
                        <div className="flex flex-wrap gap-2">
                            {project.tech.map((t, i) => (
                                <span key={i} className="px-3 py-1 border border-white/10 bg-white/5 rounded text-[10px] md:text-xs font-bold text-gray-300">{t}</span>
                            ))}
                        </div>
                    </div>
                </Link>
        ...
        <div className="mt-20 md:mt-40 text-center border-t border-white/10 pt-12 md:pt-20">
            ...
            <Link href="https://github.com" target="_blank" className="inline-flex items-center gap-3 px-8 py-4 bg-white/5 border border-white/10 rounded-full hover:bg-white hover:text-black transition-all text-sm font-bold tracking-widest">
                <Github size={20} /> {t('btn_github')}
            </Link>
        </div>
